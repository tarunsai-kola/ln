...
const AdvFullPaid = () => {
  ...
  const fetchFullPaidEnrollments = async () => {
    try {
      ...
      const filtered = enrollments.filter(
        (item) => item.status === "fullPaid" || item.status === "fullpaid"
      );
  ...
  const handleSearchChange = (event) => {
    ...
    const filtered = fullPaidEnrollments.filter((enroll) => {
      return (
    ...
    <div className="admin-content-wrap min-h-screen bg-slate-50/50 py-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">ADV Full Paid List</h1>
            <p className="text-slate-500 mt-1">View all fully paid advanced program enrollments.</p>
          </div>
                      ...
                      {!loading && (
                        <div className="flex flex-col items-center justify-center">
                          <div className="w-16 h-16 bg-slate-50 border border-slate-100 rounded-full flex items-center justify-center mb-4">
                            <i className="fa fa-money text-2xl text-slate-300"></i>
                          </div>
                          <h3 className="text-base font-bold text-slate-800">No fully paid enrollments</h3>
                          <p className="text-sm text-slate-500 mt-1 max-w-sm">There are currently no enrollments marked as fully paid.</p>
                        </div>
                      )}
                    </td>
                  </tr>
