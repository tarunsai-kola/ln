...
const AdvDefaultPayment = () => {
  ...
  const fetchAdvEnrolls = async () => {
    ...
    try {
      ...
      const defaultStudents = response.filter(
        (item) => item.status === "default" && item.operationName === operationName
      );
      setAdvEnrolls(defaultStudents);
      setFilteredStudents(defaultStudents);
      const currentMonth = getCurrentMonth();
      setSelectedMonth(currentMonth);

      const filtered = defaultStudents.filter(
  ...
  return (
    <div id="AdminAddCourse">
      <div className="coursetable">
        <h1>Advance Program - Default Payments</h1>
...
export default AdvDefaultPayment;
