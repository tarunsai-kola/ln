      ...
      {isMobileVisible && (
        <div className="sidebar premium-sidebar" style={{
          ...
          <div style={{ padding: '0 20px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', marginBottom: '16px' }}>
             <p style={{ color: '#94A3B8', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px', fontWeight: '600' }}>Operation Panel</p>
             <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, #3B82F6 0%, #2563EB 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold', fontSize: '18px', boxShadow: '0 4px 10px rgba(59, 130, 246, 0.3)' }}>
                  {operationData ? operationData.fullname.charAt(0).toUpperCase() : "O"}
                </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '0 12px' }}>
            <NavLinkItem to="/OperationDashboard" icon={Icons.Home} label="Dashboard" active={isActive("/OperationDashboard")} />
            <NavLinkItem to="/BookedPayment" icon={Icons.CalendarCheck} label="Booked Payment" active={isActive("/BookedPayment")} />
            <NavLinkItem to="/FullPayment" icon={Icons.CheckCircle} label="Full Payment" active={isActive("/FullPayment")} />
            <NavLinkItem to="/DefaultPayment" icon={Icons.AlertCircle} label="Default Payment" active={isActive("/DefaultPayment")} />
            <NavLinkItem to="/OperationRevenueSheet" icon={Icons.TrendingUp} label="Revenue Sheet" active={isActive("/OperationRevenueSheet")} />
          </div>
  ...
  return (
    <Link 
      ...
      style={{
        ...
        background: active ? 'linear-gradient(90deg, rgba(59,130,246,0.15) 0%, rgba(59,130,246,0) 100%)' : (hovered ? 'rgba(255,255,255,0.03)' : 'transparent'),
    ...
    >
      <div style={{ 
        color: active ? '#3B82F6' : (hovered ? '#CBD5E1' : '#64748B'),
